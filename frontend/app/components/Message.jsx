...
export default function Message({ type, text, answers, targetNodes, onOptionClick, source, isHistorical, isLoading }) {
  const hasAnswers = (targetNodes && targetNodes.length > 0) || (answers && answers.length > 0);
          ...
          <div className="answers-container">
            ...
            <div className="answers-list">
              {(targetNodes || answers).map((item, idx) => {
                const nodeData = typeof item === 'string' ? { text: item } : item;
                return (
                  <div key={idx} className="answer-item clickable">
                    <span className="arrow">→</span>
                    <button
                      type="button"
                      className="answer-button"
                      onClick={() => onOptionClick(nodeData.text)}
                    >
                      {nodeData.text}
                    </button>
                  </div>
