...
/**
 * Get only workflow metadata (id, name, description)
 * Optimized for initial load - no nodes or edges fetched
 */
...
/**
 * Get only node texts for a specific workflow
 * Optimized query to fetch just the node information
 */
export async function getWorkflowNodes(workflowId) {
  const query = `
    query GetWorkflowNodes($workflowId: Int!) {
  ...
  `;
  return executeGraphQL(query, { workflowId });
}
...
export async function createNode(workflowId, text, response) {
  const query = `
    mutation CreateNode($workflow_id: Int!, $text: String!, $response: String) {
      insert_nodes_one(object: {workflow_id: $workflow_id, text: $text, response: $response}) {
        id
        text
        response
        workflow_id
  ...
  `;
  return executeGraphQL(query, { workflow_id: workflowId, text, response });
}

export async function updateNode(id, text, response) {
  const query = `
    mutation UpdateNode($id: Int!, $text: String!, $response: String) {
      update_nodes_by_pk(pk_columns: {id: $id}, _set: {text: $text, response: $response}) {
        id
        text
        response
        updated_at
  ...
  `;
  return executeGraphQL(query, { id, text, response });
}
 ...
 * Optimized for performance - loads only essential data
 */
