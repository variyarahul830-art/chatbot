...
# Also track pending tasks: task_id -> client_id   // client id is generated when connect request comes for client, and task id is message id 
...
async def process_chat_message(question: str, user_id: str, session_id: str, client_id: str = None):
    ...
    """
    try:
        # Clean/normalize question text
        cleaned_question = " ".join(question.strip().split()).lower()
        logger.info(f"=== PROCESSING QUESTION ===")
        logger.info(f"Original: '{question}'")
        logger.info(f"Cleaned:  '{cleaned_question}'")
        ...
        source_node = await hasura_client.get_node_by_text(cleaned_question, workflow_id=None)
        
        if source_node:
            logger.info(f"Found exact source node: {source_node['text']}")
            target_nodes = await hasura_client.get_target_nodes(source_node["id"])
            
            logger.info(f"Found {len(target_nodes) if target_nodes else 0} target nodes")
            response_data = {
                "success": True,
                "question": source_node["text"],
                "source_response": source_node.get("response", ""),
                "target_nodes": [
                    {
                        "id": node["id"],
                        "text": node["text"].strip(),
                        "response": node.get("response", "").strip(),
                    }
                    for node in (target_nodes if target_nodes else [])
                ],
                "source": "knowledge_graph",
                "data_source": "NODE",
                "count": len(target_nodes) if target_nodes else 0
            }
            ...
            logger.info(f"📍 Returning knowledge graph response with source_response: '{response_data['source_response']}'")
            return response_data

        # Step 2: Try partial/fuzzy matching
        logger.info("📍 Step 2: Trying partial match on knowledge graph...")
        matching_nodes = await hasura_client.search_nodes_by_text(cleaned_question, workflow_id=None)
        
        if matching_nodes:
            logger.info(f"Found {len(matching_nodes)} matching nodes")
            all_target_nodes = []
            ...
            if unique_targets:
                ...
                logger.info(f"✅ Returning partial match from knowledge graph")
                return response_data
        else:
            logger.warning(f"❌ No partial matches found")
        
        # Step 3: Check Redis cache and FAQs
        logger.info("🔍 Step 3: Checking Redis cache and FAQs...")
        redis_cache = get_redis_cache()
        
        cached_answer = redis_cache.get(cleaned_question)
        if cached_answer:
            logger.info("✅ Cache hit! Answer from REDIS")
            response_data = {
                "success": True,
                "question": question,
        ...
        # Check FAQs
        faqs = await hasura_client.search_faq_partial(cleaned_question)
        if faqs and len(faqs) > 0:
            ...
            redis_cache.set(cleaned_question, {
                "answer": best_faq.get("answer"),
                "source": "faq",
                "faq_id": best_faq.get("id"),
                "category": best_faq.get("category"),
        ...
        logger.warning(f"❌ No FAQ found - falling back to RAG")
        
        # Step 4: RAG fallback - submit async task to RabbitMQ
        logger.info("🚀 Step 4: Submitting to RabbitMQ (RAG) for LLM processing...")
        try:
