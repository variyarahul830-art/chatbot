...
async def signup(request: UserSignup):
    ...
    try:
        logger.info(f"📝 Signup request: {request.username} (role: {request.role})")
        ...
        if result.get("success"):
            ...
            logger.info(f"✅ User created: {request.username} with role: {user_role}, token generated")
            return AuthResponse(
                success=True,
...
async def login(request: UserLogin):
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.info(f"✅ User logged in: {request.username} with role: {user_role}, token generated")
            return AuthResponse(
                success=True,
                message=result.get("message"),
                user=result.get("user"),
