...
async def chat(request: ChatRequest):
    ...
    """
    try:
        # Clean/trim question text to ensure exact matching
        cleaned_question = " ".join(request.question.strip().split()).lower()
        logger.info(f"Processing question: {request.question}")
        logger.info(f"Cleaned & normalized: {cleaned_question}")
        ...
        faq_exact = await hasura_client.search_faq_exact(cleaned_question)
        ...
        faq_partial = await hasura_client.search_faq_partial(cleaned_question)
        if faq_partial:
            logger.info(f"📚 Answer from FAQ")
        ...
        question_embedding = embedding_generator.generate_embedding(cleaned_question)
...
async def clear_faq_cache(user_id: str = Header(None)):
    """
