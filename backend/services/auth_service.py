...
class AuthService:
    ...
    @staticmethod
    async def signup(username: str, email: str, password: str, role: str = "user") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if role not in ["user", "admin"]:
                logger.warning(f"Signup failed: Invalid role '{role}'")
                return {
                    "success": False,
            ...
            if user:
                logger.info(f"✅ User signed up: {username} with role: {role}")
                return {
                    "success": True,
    ...
    async def login(username: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Return user info (with role, without password hash)
            user_response = {
                "id": user.get("id"),
                "username": user.get("username"),
                "email": user.get("email"),
                "is_active": user.get("is_active"),
                "role": user.get("role", "user"),  # Include role (defaults to 'user')
