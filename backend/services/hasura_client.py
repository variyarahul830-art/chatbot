...
async def update_workflow(workflow_id: int, name: str, description: Optional[str] = None):
    query = """
    ...
    """
    result = await hasura.execute(query, {"id": workflow_id, "name": name, "description": description})
    return result.get("update_workflows_by_pk")
...
async def get_node_by_text(text: str, workflow_id: Optional[int] = None):
    """Fetch a single node by its text, optionally scoped to a workflow. Uses exact match with normalization."""
    # Normalize text: trim and reduce multiple spaces to single space, lowercase
    normalized_text = " ".join(text.strip().split()).lower()
    logger.info(f"🔍 Searching for node with text: '{text}'")
    logger.info(f"📝 Normalized to: '{normalized_text}'")
    
    if workflow_id is not None:
        query = """
        query GetNodeByTextScoped($pattern: String!, $workflowId: Int!) {
            nodes(
        ...
        """
        variables = {"pattern": normalized_text, "workflowId": workflow_id}
    else:
        query = """
        query GetNodeByText($pattern: String!) {
            nodes(
        ...
        """
        variables = {"pattern": normalized_text}

    logger.info(f"🔎 Querying Hasura...")
    ...
    if nodes:
        ...
        for node in nodes:
            node_normalized = " ".join(node['text'].strip().split()).lower()
            logger.info(f"  - Node: '{node['text']}' → Normalized: '{node_normalized}'")
            if node_normalized == normalized_text:
                found_node = node
                logger.info(f"✅ EXACT MATCH FOUND - ID: {found_node['id']}, Response: '{found_node.get('response', '')}'")
                return found_node
        
        # If no exact match, return the first one anyway
        logger.warning(f"⚠️ No exact match found, using first result")
        return nodes[0]
    else:
        logger.warning(f"❌ No node found for: '{text}'")
...
async def create_node(workflow_id: int, text: str, response: Optional[str] = None):
    query = """
    mutation CreateNode($workflowId: Int!, $text: String!, $response: String) {
        insert_nodes_one(object: {workflow_id: $workflowId, text: $text, response: $response}) {
            id
            text
            response
            workflow_id
    ...
    """
    result = await hasura.execute(query, {"workflowId": workflow_id, "text": text, "response": response})
    return result.get("insert_nodes_one")


async def update_node(node_id: int, text: Optional[str] = None, response: Optional[str] = None):
    ...
    query = """
    mutation UpdateNode($id: Int!, $set: nodes_set_input!) {
        update_nodes_by_pk(pk_columns: {id: $id}, _set: $set) {
            id
            text
            response
            updated_at
    ...
    """
    result = await hasura.execute(query, {"id": node_id, "set": update_fields})
    return result.get("update_nodes_by_pk")
...
async def get_faq_questions():
    """Get only FAQ questions (without answers) for suggestions"""
    query = """
...
async def get_user_chat_sessions(user_id: str):
    ...
    query GetUserChatSessions($userId: String!) {
        chat_sessions(
            where: {user_id: {_eq: $userId}, is_active: {_eq: 1}}
...
async def create_user(username: str, email: str, password_hash: str, role: str = "user"):
    """Create new user with optional role"""
    query = """
    mutation CreateUser($username: String!, $email: String!, $password_hash: String!, $role: String) {
        insert_users_one(object: {username: $username, email: $email, password_hash: $password_hash, role: $role, is_active: 1}) {
            id
            username
            email
            role
...
async def get_user(user_id):
    """
    Get user by ID (flexible - accepts string or int)
    Used by role_check middleware for verification
    """
    try:
        # Convert string user_id to int if needed
        user_id_int = int(user_id) if isinstance(user_id, str) else user_id
        return await get_user_by_id(user_id_int)
